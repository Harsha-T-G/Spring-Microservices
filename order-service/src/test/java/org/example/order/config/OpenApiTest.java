...
class OpenApiTest {
    ...
    void givenRunningService_whenReadingOpenApi_thenBusinessContractAndServiceIdentityAreAvailable() throws Exception {
        mvc.perform(get("/v3/api-docs")).andExpect(status().isOk())
                .andExpect(jsonPath("$.info.title").value("Order API"))
    ...
    void givenApiConsumer_whenInspectingPost_thenHeadersExamplesAndOutcomeSchemasAreUsable() throws Exception {
        mvc.perform(get("/v3/api-docs")).andExpect(status().isOk())
                .andExpect(jsonPath("$.paths['/api/v1/orders'].post.parameters[?(@.name == 'Idempotency-Key')].required")
                        .value(org.hamcrest.Matchers.contains(true)))
                .andExpect(jsonPath("$.paths['/api/v1/orders'].post.parameters[?(@.name == 'X-Correlation-Id')].required")
                        .value(org.hamcrest.Matchers.contains(false)))
                .andExpect(jsonPath("$.paths['/api/v1/orders'].post.responses['201'].content['application/json'].schema['$ref']")
                        .value("#/components/schemas/OrderResponse"))
                .andExpect(jsonPath("$.paths['/api/v1/orders'].post.responses['400'].content['application/json'].schema['$ref']")
                        .value("#/components/schemas/ApiError"))
                .andExpect(jsonPath("$.components.schemas.CreateOrderRequest.example.quantity").value(2));
    }
