...
public class OpenApiConfiguration {
    ...
    public OpenAPI serviceOpenApi(@Value("${info.app.version}") String version) {
        return new OpenAPI().info(new Info().title("Order API").version(version)
                .description("Create and inspect orders. Inventory must be running to confirm a new order."));
    }
    @Bean
